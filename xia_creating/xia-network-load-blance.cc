...
NS_LOG_COMPONENT_DEFINE("GENERIC_SIMULATION"); //定义日志组件标签，运行时可以通过该标签决定是否打印输出 NS_LOG=GENERIC_SIMULATION=debug
...
uint32_t cc_mode = 1;           // mode for congestion control, 1: DCQCN；3：hpcc；7：hpcc
...
//二维的map可以理解为二维索引，行-列索引，value就是最里面的一层value
//可以看成多个矩阵，源-目的节点为行列索引，内容分别为：下一跳节点，延迟，带宽，rtt，bdp
...
std::map<uint32_t, std::vector<uint32_t>> torId2UplinkIf; //ToR#id的上行接口编号
std::map<uint32_t, std::vector<uint32_t>> torId2DownlinkIf;//ToR#id的下行接口编号

// input files
std::ifstream topof, flowf;              //拓扑文件和流文件
NodeContainer n;                         // node container ns3中的网络节点容器，用于管理node
std::vector<Ipv4Address> serverAddress;  // server address ns3中提供的ipv4地址，vector可以管理所有节点的ip地址

// flow generator
std::unordered_map<uint32_t, uint32_t> flows_per_host;  //键：主机ID，值：流数
uint32_t flow_id = 0;                                   //全局流编号
std::unordered_map<uint32_t, uint16_t> portNumber;      //键：节点id，值：源端口号
std::unordered_map<uint32_t, uint16_t> dportNumber;     //键：节点id，值：目的端口号
uint16_t *port_per_host; //

// Scheduling input flows from flow.txt
struct FlowInput {
    uint32_t src, dst, pg, maxPacketCount, port;       //pg在流文件中就是3，rdma应用需要？？
    double start_time;
    uint32_t idx;
};
 ...
 */
void ReadFlowInput() { //只能用./config/L_25.00_CDF_AliStorage2019_N_128_T_100ms_B_100_flow.txt的格式才符合要求
...
void ScheduleFlowInputs(FILE *infile) {
    ...
    while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()) {
        ...
        // src port
        sport = portNumber[src];  // get a new port number //根据源节点得到端口号
        portNumber[src] = portNumber[src] + 1; //端口号自增

        // dst port
        dport = dportNumber[dst]; //根据目的节点map表得到目的端口号
        dportNumber[dst] = dportNumber[dst] + 1; //端口号自增

        target_len = flow_input.maxPacketCount;  // this is actually not packet-count, but bytes //不规范的命名，其实是字节数
        if (target_len == 0) {
            target_len = 1;
        }
        assert(n.Get(src)->GetNodeType() == 0 && n.Get(dst)->GetNodeType() == 0);//switchnode=1,hostnode=0
        ...
        if (pairRtt.find(n.Get(src)) == pairRtt.end() ||
            ...
            assert(false);
        } //得到源节点容器，find到rtt值在map表内，

        RdmaClientHelper clientHelper(
            pg, serverAddress[src], serverAddress[dst], sport, dport, target_len,
            has_win ? (global_t == 1 ? maxBdp : pairBdp[n.Get(src)][n.Get(dst)]) : 0, //has_win=1,global_t=1,直接用全局最大BDP，否则用特定节点之间的BDP
            global_t == 1 ? maxRtt : pairRtt[n.Get(src)][n.Get(dst)]); //用全局最大Rtt
        clientHelper.SetAttribute("StatFlowID", IntegerValue(flow_input.idx)); //创建一个客户端，指定服务器地址、端口、流大小、窗口大小等参数

        ApplicationContainer appCon = clientHelper.Install(n.Get(src));  // SRC 安装到源节点
        appCon.Start(Seconds(Time(0)));
        appCon.Stop(Seconds(100.0));

        flow_input.idx++; //读取下一条流
        ReadFlowInput();
    }
 ...
 */
//达到一定流数量后自动结束监控，而不必手动指定 Simulator::Stop(100s) 之类的固定终止时
void stop_simulation_middle() {
    uint32_t target_flow_num = flow_num - 0;  // can be lower than flownum，0是个占位符，修改为其他数可以表示提前终止
    if (Settings::cnt_finished_flows >= target_flow_num) { //已完成的流数大于目标流数，结束仿真，打印相关信息
        ...
        Simulator::Stop(NanoSeconds(1));  // finish soon, stop this schedule (NECESSARY!) NS3要求延迟一点点结束，直接结束会导致系统调度混乱
        return;
    }

    Simulator::Schedule(MicroSeconds(100), &stop_simulation_middle);  // check every 100us 每100微秒检查一次调度有无结束
}

/**
 * @brief Calculate edge-to-edge delays, TX delays, and bandwidths
 */
//该函数是为每个host执行，可以发现所有可达host节点的最短距离dis，nexthop表（从不同交换机到host的下一跳）；
void CalculateRoute(Ptr<Node> host) {
    ...
    // BFS. 按照层级从近到远，适合大范围寻找，队列形式，先进先出；DFS优先走到底再回溯，适合目标明确
    for (int i = 0; i < (int)q.size(); i++) {
        Ptr<Node> now = q[i];
        int d = dis[now];
        for (auto it = nbr2if[now].begin(); it != nbr2if[now].end(); it++) { //it是迭代器，first是邻居节点，second是interface结构体
            // skip down link
            if (!it->second.up) continue; //Interface是up才可用
            Ptr<Node> next = it->first; //把邻居节点给next
            // If 'next' have not been visited.
            if (dis.find(next) == dis.end()) { //如果next节点不在dis中
                dis[next] = d + 1; //到next的距离+1
                delay[next] = delay[now] + it->second.delay;  // maybe nanoseconds? 传播时延
                txDelay[next] = txDelay[now] + packet_payload_size * 1000000000lu * 8 /
                                                   it->second.bw;  // maybe nanoseconds?到next的传输时延=包大小/带宽
                bw[next] = std::min(bw[now], it->second.bw); //带宽为小值
                // we only enqueue switch, because we do not want packets to go through host as
                // middle point
                if (next->GetNodeType() == 1) { //如果是交换机类型，入队
                    q.push_back(next);
                }
            }
            // if 'now' is on the shortest path from 'next' to 'host'.
            if (d + 1 == dis[next]) { //发现等长路径
                nextHop[next][host].push_back(now); //从next交换机到host的的元素vector，表示中间跳；第一层key是next，第二层key是host
            }
        }
    }
    for (auto it : delay) {
...
void SetRoutingEntries() {
    ...
    for (auto i = nextHop.begin(); i != nextHop.end(); i++) {
        Ptr<Node> node = i->first; //nextHop<[switch],<[host],vector>>所以first是主机/交换机节点
        auto &table = i->second; //table是<[host],vector>
        for (auto j = table.begin(); j != table.end(); j++) {
            // The destination node.
            Ptr<Node> dst = j->first; //host
            // The IP address of the dst.
            Ipv4Address dstAddr = dst->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(); //得到host的ipv4地址
            // The next hops towards the dst.
            vector<Ptr<Node>> nexts = j->second; //可以到该host的所有节点
            for (int k = 0; k < (int)nexts.size(); k++) {
                Ptr<Node> next = nexts[k];
                uint32_t interface = nbr2if[node][next].idx; //node到交换机的idx
                if (node->GetNodeType() == 1)
                    DynamicCast<SwitchNode>(node)->AddTableEntry(dstAddr, interface);//seitch安装路由表
                else {
                    node->GetObject<RdmaDriver>()->m_rdma->AddTableEntry(dstAddr, interface);//host安装RDMA
                }
            }
        }
    }
 ...
 */
//把 a 和 b 之间的链路设为不可用（down）清除原有路径和路由表；根据新拓扑（少了一条边）重新计算所有路径
//重建路由表；通知底层接口关闭；重新分配 RDMA 的连接资源（QP）
...
int main(int argc, char *argv[]) { //需要传入配置文件路径
...
#endif
    {
        ...
        std::ifstream conf;
#ifndef PGO_TRAINING
        conf.open(argv[1]); //打开配置文件
#else
        conf.open(PATH_TO_PGO_CONFIG);
#endif
        while (!conf.eof()) { //持续读取键值对
            std::string key;
            conf >> key;
    ...
    for (uint32_t i = 0; i < switch_num; i++) { //从拓扑配置中读取switch_num个编号
        uint32_t sid;
        topof >> sid;
        node_type[sid] = 1; //标记为交换机
    }
    for (uint32_t i = 0; i < node_num; i++) {
        if (node_type[i] == 0)
            n.Add(CreateObject<Node>()); //主机节点
        else {
            Ptr<SwitchNode> sw = CreateObject<SwitchNode>(); //交换机节点
            n.Add(sw);
            sw->SetAttribute("EcnEnabled", BooleanValue(enable_qcn)); //控制是否弃用QCN机制
        }
    }
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            serverAddress[i] = Settings::node_id_to_ip(i); //给编号为i的主机分配一个ip地址
        }
    }
    ...
    //
    Ptr<RateErrorModel> rem = CreateObject<RateErrorModel>(); //创建链路误码模型
    Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable>(); //使用均匀分布随机数生成器
    rem->SetRandomVariable(uv);
    uv->SetStream(50);
    rem->SetAttribute("ErrorRate", DoubleValue(error_rate_per_link));
    rem->SetAttribute("ErrorUnit", StringValue("ERROR_UNIT_PACKET"));

    pfc_file = fopen(pfc_output_file.c_str(), "w"); //PFC相关日志写入文件
    ...
    for (uint32_t i = 0; i < link_num; i++) {
        ...
        if (snode->GetNodeType() == 0) {
            ...
            ipv4->AddInterface(d.Get(0)); //主机节点手动设置接口编号为1，并分配静态IP
        ...
        char ipstring[16];
        Ipv4Address x;
        sprintf(ipstring, "10.%d.%d.0", i / 254 + 1, i % 254 + 1); //为链路分配一个子网地址
